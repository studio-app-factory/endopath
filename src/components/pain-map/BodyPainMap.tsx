...
export function BodyPainMap({
  ...
  return (
    <div className="relative">
          ...
          (Object.keys(ZONE_HITAREAS) as BodyZone[]).map((z) => (
            <button
              ...
              className={`absolute rounded-md transition-colors cursor-pointer ${
                selectedZone === z
                  ? 'bg-rose-400/25 ring-1 ring-rose-300/60'
                  : 'hover:bg-rose-400/15 active:bg-rose-400/25'
              }`}
        ...
        {locations.map((loc) => {
          ...
          return (
            <button
              ...
              className="absolute -translate-x-1/2 -translate-y-1/2 w-7 h-7 rounded-full text-[11px] font-bold text-[#1A0E13] shadow-md flex items-center justify-center cursor-pointer hover:scale-110 transition-transform"
      ...
      {/* Zone selector modal */}
      {selectedZone && (
        <div className="mt-4 p-5 bg-[#241318] rounded-3xl border border-rose-400/25 shadow-xl shadow-rose-400/15 animate-in slide-in-from-bottom-4">
          <div className="flex items-center justify-between mb-3">
            <p className="text-base font-medium text-white font-['Cormorant_Garamond']">
              Pain in:{' '}
              <span className="bg-gradient-to-br from-rose-400 to-amber-400 bg-clip-text text-transparent font-semibold">
                {ZONE_LABEL[selectedZone]}
              </span>
            </p>
            <button
              onClick={() => setSelectedZone(null)}
              className="w-7 h-7 rounded-full bg-white/6 hover:bg-white/10 flex items-center justify-center cursor-pointer"
            >
          ...
          <div className="space-y-2 mb-4">
            <div className="flex justify-between text-[10px] text-white/55 uppercase tracking-wider px-1">
              <span>Mild</span>
              <span>Moderate</span>
              <span>Severe</span>
            </div>
            ...
            <div className="text-center">
              <span
                className="inline-flex items-center justify-center w-11 h-11 rounded-full text-[#1A0E13] font-bold text-base shadow-lg shadow-rose-400/35"
          ...
          <div className="flex gap-3">
            <button
              onClick={() => setSelectedZone(null)}
              className="flex-1 py-2.5 rounded-2xl border border-white/12 text-white/65 text-sm font-medium hover:bg-white/6 transition-colors cursor-pointer"
            >
              Cancel
            </button>
            <button
              onClick={handleConfirm}
              className="flex-1 py-2.5 rounded-2xl bg-gradient-to-br from-rose-400 to-amber-400 text-[#1A0E13] text-sm font-semibold shadow-lg shadow-rose-400/30 hover:shadow-xl transition-all cursor-pointer inline-flex items-center justify-center gap-1.5"
            >
